...
impl<P: VectorSpace<Scalar = f32>> CubicCurve<P> {
    ...
    ///
    /// This spline's sge_global `t` value is equal to how many segments it has.
    ///
    /// All method accepting `t` on `CubicCurve` depends on the sge_global `t`.
    ...
    /// Returns the [`CubicSegment`] and local `t` value given a spline's sge_global `t` value.
    #[inline]
...
impl<P: VectorSpace<Scalar = f32>> RationalCurve<P> {
    ...
    /// This spline's sge_global `t` value is equal to how many segments it has.
    ///
    /// All method accepting `t` on `RationalCurve` depends on the sge_global `t`.
    /// When sampling over the entire curve, you should either use one of the
    /// `iter_*` methods or account for the segment count using `curve.segments().len()`.
    #[inline]
    ...
    /// Returns the [`RationalSegment`] and local `t` value given a spline's sge_global `t` value.
