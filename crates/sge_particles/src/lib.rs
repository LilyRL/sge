...
impl ParticleSystem {
    ...
    pub fn spawn_batch(&mut self, batch: &ParticleBatch<impl Shape2DExt + 'static>, pos: Vec2) {
...
#[bon]
impl<T: Shape2DExt + 'static> ParticleBatch<T> {
    ...
    fn generate(&self, vec: &mut Vec<Particle>, pos: Vec2) {
        ...
        for _ in 0..quantity as usize {
            let direction = self.direction + (rand::<f32>() - 0.5) * self.direction_randomness;
